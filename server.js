...
io.sockets.on('connection', function (socket) {
  ...
  socket.on("postingform", function (data){
      // console.log('submit!: ' + data.message.yourname);
      console.log('You emitted the following information to the server: ' + JSON.stringify(data));
      socket.emit('updatedmessage', data);
      var randomnumber = 1 + Math.floor(Math.random()*1000);
      console.log('Your lucky number emitted by the server is: '+randomnumber);
